...
def koliko(niz,datoteka):
    #niz razdelimo glede na , v seznam imen
    seznam_imen=niz.split(",")
    #naredimo seznam brez ponovitev
    seznam_brez_ponovitev=[]
    for i in seznam_imen:
        if i not in seznam_brez_ponovitev:
            seznam_brez_ponovitev.append(i)
    #odpermo iyhodno dat
    with open(datoteka, "w") as f:
        for ime in seznam_brez_ponovitev:
            kolikokrat_se_pojavi = seznam_imen.count(ime)
            print("{} {}".format(ime, kolikokrat_se_pojavi), file=f)
    
       
    
        #za vsako ime izračunamo kolikokrat se pojavi in to zapišemo v izhodno datoteko
...
def koliko_urejen(vhodna, izhodna):
    ...
    #prirpravimo nov seznam po pravem vrsten redu
    pari=[]
    for ime in seznam_brez_ponovitev:
        kolikokrat_se_pojavi = seznam_imen.count(ime)
        pari.append((-kolikokrat_se_pojavi,ime))
    pari.sort()
        
    #odpermo iyhodno dat
    with open(izhodna, "w") as f:
        for n,ime in pari:
            print("{} {}".format(ime, -n), file=f)
