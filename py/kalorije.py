...
def kalorije_na_dan(ime_datoteke):
    with open(ime_datoteke) as f:
        rezultat=[]
        for vrstica in f:
            danasnje_kalorije=sum(vrni_kalorije(vrstica.strip()))
            rezultat.append(danasnje_kalorije)
    return(rezultat)
# =====================================================================@001516=
# 3. podnaloga
...
def vsota_kalorij(ime_vhodne,ime_izhodne):
    ...
    with open(ime_izhodne, "w") as f: #w pomeni pisi
        for x in seznam_kalorij:
            print(x, file=f) #pise v datoteko
    return
...
def povprecje_kalorij(ime_vhodne, ime_izhodne):
    seznam_vsot_kalorij_po_dnevih=[] #dnevne vsote
    stevilka_dneva=0
    
    #odpremo vhodno in gremo po vrsticah 
    with open(ime_vhodne) as f:
        #odpremo se izhodno
        with open(ime_izhodne, "w") as g:
            #gremo po vrsticah vhodne
            for vrstica in f:
                
                #iz vrstice dobimo podatke o kalorijah
                seznam_dnevnih_kalorij=vrni_kalorije(vrstica.strip())
                
                #izračunamo povprečje
                vsota_dnevnih_kalorij=sum(seznam_dnevnih_kalorij)
                dnevno_povprecje=vsota_dnevnih_kalorij/len(seznam_dnevnih_kalorij)
                
                #dodamo vsoto dnevnih kalorij v seznam kalorij po dnevih
                seznam_vsot_kalorij_po_dnevih.append(vsota_dnevnih_kalorij)

                #povečamo številko dneva
                stevilka_dneva += 1

                #zapišemo v izhodno datotek
                niz="{} {:.2f}".format(stevilka_dneva,dnevno_povprecje)
                print(niz,file=g)
                
            #povprečje dnevno zaužitih kalorij
            povprecje=sum(seznam_vsot_kalorij_po_dnevih)/len(seznam_vsot_kalorij_po_dnevih)
            print("{:.2f}".format(povprecje),file=g)
...
def _validate_current_file():
    ...
    try:
        ...
        token = 'Token 2c6c09389bfd3a01a32b8792ddd792784fde75ff'
